...
var book = {
    ...
    //以下四个方法可能都不需要了，修改 moveCanvas 的实现，以通用自动翻页
    //自动下一页
    nextPage:function(){
    },
    //自动前一页
    ...
    autoMoveToLeft:function(){
        ...
        var si = setInterval(function(){
            ...
            _self.setPositionAuto({"x":pos.d1.x-stepx,"y":pos.d1.y+stepy});
            _self.moveCanvas();
            if(_self.ifNext){
                clearInterval(si);
    ...
    autoMoveToRight:function(){
        ...
        var si = setInterval(function(){
            ...
            var x = pos.d1.x+stepx;
            var y = pos.d1.y+stepy ;
            
            //如果有一个符号变化，说明到头了
            if((pfx - pos.d1.x >0)!=markx || (pfy - pos.d1.y >0)!=marky){
                x = pfx;
                y = pfy;
            }
            _self.setPositionAuto({"x":x,"y":y});
            _self.moveCanvas();
            if(pos.d1.x >= pfx && pos.d1.y >= pfy){
                _self.clearPage();
                clearInterval(si);
            }
            if(_self.ifNext){
    ...
    autoChangePage:function(){//接收一个参数，判断热区来源，保留，未开发
        ...
        //不再需要确定从哪里激活，只是让页面自动移动到左侧或右侧
            if(pos.d1.x > pcx){
                _self.autoMoveToRight();
            }else{
                _self.autoMoveToLeft();
            }
        //}
    },
    //移动至下一页
    ...
    //清除阴影时使用，即翻页完成时,参数：页码
    clearPage:function(i){
        var _self=this;
        var stage = _self.stage;
        _self.context.clearRect(0, 0, _self.settings.stage.width, _self.settings.stage.height);
        _self.context.drawImage(_self.pagesImage[_self.currentPage],stage.pax,stage.pay,_self.settings.shape.width,_self.settings.shape.height);
    },
    /****************************************************************************************************/
    /*                                                  算法核心                                        */
    /****************************************************************************************************/
