...
var book = {
    ...
    drawNextPage:function(curPage){
        ...
        if(pos.d3.x == pos.d4.x && pos.d3.y == pos.d4.y){
            ctxNext.moveTo(stage.pax,pos.d3.y);
            ctxNext.lineTo(stage.pbx,stage.pby);
            ctxNext.lineTo(pos.d0.x-(pos.d2.x-pos.d0.x),pos.d2.y);
            ctxNext.lineTo(stage.pax,pos.d3.y);
        }else{
            ctxNext.moveTo(stage.pax,stage.pay);
        ...
        ctxNext.clip();
        //绘图
        ctxNext.drawImage(_self.pagesImage[_self.currentPage+1],stage.pax,stage.pax,_self.settings.shape.width,_self.settings.shape.height);
        
        //将临时图形绘制到主画布
        var ctx = _self.context;
        var l=Math.cos(angle)*(pos.d0.x-(pos.d2.x-pos.d0.x)-stage.pax);
        var t=pos.d0.x-(pos.d2.x-pos.d0.x)-stage.pax-l;
        ctx.drawImage(_self.canvasNextPage[0],pos.d1.x-stage.pax-t,0,_self.settings.stage.width,_self.settings.stage.height);
        ctxNext.restore();
    },
    //在右侧绘制当前页
    drawCurrentPage:function(curPage){
        ...
        if(pos.d3.x == pos.d4.x && pos.d3.y == pos.d4.y){
            ctxNext.moveTo(stage.pax,stage.pay);
            ctxNext.lineTo(stage.pex,stage.pey);
            ctxNext.lineTo(pos.d3.x,pos.d3.y);
            ctxNext.lineTo(pos.d2.x,pos.d2.y);
            ctxNext.lineTo(stage.pbx,stage.pby);
            ctxNext.lineTo(stage.pax,stage.pay);
        }else{
            ctxNext.moveTo(stage.pax,stage.pay);
            ctxNext.lineTo(pos.d3.x,pos.d3.y);
            ctxNext.lineTo(pos.d2.x,pos.d2.y);
        ...
        ctxNext.clip();
        ctxNext.drawImage(_self.pagesImage[_self.currentPage],stage.pax,stage.pax,_self.settings.shape.width,_self.settings.shape.height);
        ctxNext.restore();
